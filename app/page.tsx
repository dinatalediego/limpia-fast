...
const faq = [
  ...
  ['¿Cuánto tarda en secar?', 'Depende del material, la ventilación y las condiciones del día. El equipo te dará una estimación para tu caso antes o durante la coordinación.'],
...
export default function Home() {
  ...
  const schema = {
    '@context': 'https://schema.org', '@type': 'LocalBusiness', name: 'LimpiaFast',
    description: 'Lavado de muebles, colchones, alfombras y tapicería a domicilio.', telephone: '+51 993 984 874',
    areaServed: { '@type': 'City', name: 'Huacho' }, url: process.env.NEXT_PUBLIC_SITE_URL || 'https://limpia-fast.vercel.app',
  ...
  return <main>
    ...
    <section className="hero" id="inicio">
      ...
      <div className="hero-copy">
        <div className="local-badge"><span /> Atención a domicilio · Huacho</div>
    ...
    <section className="services section-shell" id="servicios"><div className="section-head"><div><p className="section-kicker">Qué puedes recuperar</p><h2>Empieza por el resultado que quieres sentir.</h2></div><p>No organizamos la oferta como un catálogo técnico. Elige el mueble que hoy te está generando fricción.</p></div><div className="service-list">{services.map((service) => <article key={service.code} className="service-row" onMouseEnter={() => trackEvent('service_viewed', { service: service.title })}><span className="service-code">{service.code}</span><div><h3>{service.title}</h3><p>{service.text}</p></div><span className="service-detail">{service.detail}</span><a href="#cotizar" aria-label={`Cotizar ${service.title}`} onClick={() => trackEvent('CTA_click', { origin: 'service', service: service.title })}>↘</a></article>)}</div></section>

    <section className="quote-zone" id="cotizar"><div className="quote-copy"><p className="section-kicker light">Cotiza sin empezar de cero en un chat</p><h2>Cuatro pasos. Menos preguntas después.</h2><p>El formulario recoge exactamente el contexto que el flujo comercial necesita: qué quieres limpiar, tamaño, distrito, problema y urgencia.</p><div className="data-cards"><div><span>01</span><strong>Más contexto</strong><p>Una cotización mejor empieza con datos útiles.</p></div><div><span>02</span><strong>Menos fricción</strong><p>WhatsApp queda para la foto y el cierre, no para repetir lo básico.</p></div><div><span>03</span><strong>Más señal</strong><p>Cada solicitud conserva su fuente y campaña para aprender qué tráfico sí convierte.</p></div></div></div><LeadWizard /></section>

    <section className="process section-shell" id="proceso"><div className="section-head"><div><p className="section-kicker">De intención a servicio</p><h2>Sabes qué pasa en cada momento.</h2></div><p>La confianza mejora cuando la siguiente acción es evidente y no hay sorpresas entre “quiero cotizar” y “ya quedó reservado”.</p></div><div className="process-track"><article><span>01</span><div className="process-visual p1" /><h3>Describe</h3><p>Tipo, tamaño, distrito y qué quieres resolver.</p></article><article><span>02</span><div className="process-visual p2" /><h3>Muestra</h3><p>Envía una foto para reducir ambigüedad sobre material y estado.</p></article><article><span>03</span><div className="process-visual p3" /><h3>Confirma</h3><p>Revisamos el caso y coordinamos precio, cobertura y horario.</p></article><article><span>04</span><div className="process-visual p4" /><h3>Recupera</h3><p>El servicio ocurre a domicilio según lo coordinado.</p></article></div></section>

    <section className="certainty section-shell"><div className="certainty-card"><p className="section-kicker light">Lo que sí prometemos en la web</p><h2>Claridad antes de venderte.</h2><div className="certainty-grid"><div><span>✓</span><p>La cotización parte de información real de tu mueble.</p></div><div><span>✓</span><p>La cobertura se confirma antes de reservar.</p></div><div><span>✓</span><p>No publicamos cifras o garantías que aún no estén validadas.</p></div><div><span>✓</span><p>Puedes empezar en la web o ir directo a WhatsApp.</p></div></div><WhatsAppLink origin="certainty" className="button button-light">Hablar con LimpiaFast <span aria-hidden="true">↗</span></WhatsAppLink></div><div className="certainty-aside"><span className="giant-quote">“</span><p>Antes de cambiarlo, veamos si podemos recuperarlo.</p><small>Concepto de “rescate” en validación dentro del Market Research de LimpiaFast.</small></div></section>
